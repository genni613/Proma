...
export const AgentMessages = React.memo(function AgentMessages({
  ...
  const minimapItems: MinimapItem[] = React.useMemo(
    () => {
      const turns = toTranscript(structuralGroups)
      return structuralGroups.map((group, index) => {
  ...
  React.useEffect(() => {
    if (streaming || tabMinimapItems.length === 0) return
    setMinimapCache((prev) => {
      const next = new Map(prev)
      next.set(sessionId, tabMinimapItems)
      return next
    })
  }, [sessionId, tabMinimapItems, streaming, setMinimapCache])
